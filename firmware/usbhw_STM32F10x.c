...
void USB_Init (void) {
    ...
    RCC->APB2ENR |= (1 << 0);                   /* enable clock for USB */
 ...
 *    Return Value:    None
 */
#define USB_CONNECT_PIN                     12
#define USB_CONNECT_PIN_SET                 (1 << (USB_CONNECT_PIN))            // BSRR
#define USB_CONNECT_PIN_MODE_CLR            (~(3 << (USB_CONNECT_PIN * 2)))     // MODER
#define USB_CONNECT_PIN_OUTPUT              (1 << (USB_CONNECT_PIN * 2))        // MODER
...
{ 
    if (con) {
        ...
        int i = 0;
        unsigned long mode = GPIOA->MODER;
