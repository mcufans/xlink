...
static  uint8_t  USB_Request [XLINK_PACKET_COUNT][XLINK_PACKET_SIZE];  // Request  Buffer
static  uint8_t  USB_Response[XLINK_PACKET_COUNT][XLINK_PACKET_SIZE];  // Response Buffer
static  uint8_t USB_ResponseLen[XLINK_PACKET_COUNT];
...
{   
    if (event == USB_EVT_OUT) {     // 接收处理
        // 仅当缓冲列表有空闲项时才读取数据
        if (USB_RequestCount < XLINK_PACKET_COUNT) {
            USB_ReadEP(XLINK_CMD_ENDP_IN, USB_Request[USB_RequestIn]);
            
            // 增加读取的包计数和写索引
            USB_RequestIn++;                            
            if (USB_RequestIn == XLINK_PACKET_COUNT) {
                USB_RequestIn = 0;
            }
            USB_RequestCount++;
        }
        ...
        if (USB_ResponseCount) {
            ...
            // 纠正读索引和包计数
            USB_ResponseOut++;
            if (USB_ResponseOut == XLINK_PACKET_COUNT) {
              USB_ResponseOut = 0;
            }
            USB_ResponseCount--;
        } else {
...
void USB_ProcessCommand (void) 
{
    ...
    if (USB_RequestCount) {
        resp_len = ProtocolProcCommand(USB_Request[USB_RequestOut], USB_Response[USB_ResponseIn]);
        
        USB_ResponseLen[USB_ResponseIn] = resp_len; 
        
        // 校正请求响应写索引和包计数
        n = USB_RequestOut + 1;
        if (n == XLINK_PACKET_COUNT) {
            n = 0;
        }
        USB_RequestOut = n;
        USB_RequestCount--;
